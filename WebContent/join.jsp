...
<body>
 ...
 function checkMember() {
		var regExpUserID = /^[a-z|A-Z|ㄱ-ㅎ|ㅏ-ㅣ|가-힣]/;
		var regExpUserPassword = /^[0-9]*$/;
		var regExpUserEmail = /^[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*\.[a-zA-Z]{2,3}$/i;
		...
		var userEmail = form.userEmail.value;
			...
			if (!regExpUserPassword.test(userPassword)) {
			alert("비밀번호는 숫자만입력해주세요!");
			form.userPassword.select();
			return false;
		}
		if (!regExpUserEmail.test(userEmail)) {
   ...
   <section class="container mt-3" style="max-width: 560px;">
    <form name="Member" method="post" action="./joinAction.jsp">
     <h3 style="text-align: center;">회원가입 화면</h3>
     <div title="4~20자 이내로 작성해주세요" class="form-group">
      <input type="text" class="form-control" placeholder="아이디" name="userID" maxlength="20">
      </div>
      <div title="숫자로만 구성해주세요" class="form-group">
      <input type="password" class="form-control" placeholder="비밀번호" name="userPassword" maxlength="20">
      </div>
      	<div title="아이디@~~.~~ 양식에 맞게 써주세요" class="form-group">
      <input type="text" class="form-control" placeholder="이메일" name="userEmail" maxlength="20">
      </div>
      <input type="submit" class="btn btn-primary form-control" value="회원가입" onclick="checkMember()">
    </form>
  </section>
